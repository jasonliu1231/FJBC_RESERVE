  ...
  switch (req.method) {
    case "POST":
      ...
      try {
        ...
        if (!Array.isArray(shopCartList) || shopCartList.length === 0) {
          return res.status(400).json({ error: "Invalid shopCartList data" });
        }
        ...
        for (const [index, product] of shopCartList.entries()) {
          ...
          if (Array.isArray(comboProductList) && comboProductList.length > 0) {
